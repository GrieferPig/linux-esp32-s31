...
{
	/* hci_register_dev() opens the controller once for setup and closes it
	 * again.  Propagate that power transition to BTDM so an idle hci0 does
	 * not keep the coexistence scheduler taking airtime from Wi-Fi. */
	return esp32s31_radio_bt_disable();
}
